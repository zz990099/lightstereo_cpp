...
TEST(lightstereo_test, ort_core_correctness)
{
...
TEST(lightstereo_test, ort_core_speed)
{
  auto model         = CreateLightStereoModel();
  auto [left, right] = ReadTestImages();
...
TEST(lightstereo_test, ort_core_pipeline_correctness)
{
  auto model = CreateLightStereoModel();
  model->InitPipeline();
  auto [left, right] = ReadTestImages();

  auto async_func = [&]() { return model->ComputeDispAsync(left, right); };
...
TEST(lightstereo_test, ort_core_pipeline_speed)
{
  auto model = CreateLightStereoModel();
  model->InitPipeline();
  auto [left, right] = ReadTestImages();
